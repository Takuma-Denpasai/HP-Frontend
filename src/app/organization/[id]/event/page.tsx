...
import { faPlus, faUser, faCircleCheck, faCircleExclamation, faCircleXmark, faSpinner, faCalendar } from '@fortawesome/free-solid-svg-icons';
...
interface Event {
  ...
  event_inspections__ai: boolean;
  event_inspections__inspected: boolean;
  event_inspections__deleted: boolean;
  created_at: string;
  updated_at: string;
}
        ...
        <main>
            <div className="mx-3.5 my-10">
                    ...
                    {event && event.map((event) => (
                      <Link key={event['id']} href={`/organization/${params.id}/event/${event['id']}`}>
                        <div className="w-full p-4 bg-white rounded-lg py-6 my-4 hover:text-gray-600 transition duration-100">
                          ...
                          {event['event_inspections__deleted'] ? (
                            <p className="text-xs my-1.5 text-red-600"><FontAwesomeIcon icon={faCircleXmark} /> 検証によって削除済み</p>
                          ) : (
                          event['event_inspections__ai'] ? (
                            event['event_inspections__inspected'] ? (
                ...
                <Link href={`/organization/${params.id}/event/new`}>
                    <p className="text-center text-white hover:text-gray-200 transition duration-100 text-base"><FontAwesomeIcon icon={faPlus} /> イベント追加</p>
                </Link>
            </div>
